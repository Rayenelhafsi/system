...
// Liste des types de biens pour lesquels la caractéristique peut être pertinente
$typesBiens = [
    "Appartement",
    "Villa",
    "Terrain",
...
// Récup valeurs formulaire (pour garder ce que l'utilisateur a coché en cas d'erreur)
$nom   = trim($_POST['nom'] ?? '');
$types = $_POST['types'] ?? []; // tableau de types cochés
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nom === '') {
        ...
        // Normaliser pour éviter les doublons casse / espaces
        $nomCheck = trim(mb_strtolower($nom));

        // Vérifier doublon (insensible à la casse)
        $check = $pdo->prepare("SELECT id, nom FROM caracteristiques WHERE LOWER(nom) = ?");
        $check->execute([$nomCheck]);

        if ($check->rowCount() > 0) {
            $row = $check->fetch(PDO::FETCH_ASSOC);
            $error = "Cette caractéristique existe déjà : <strong>" . htmlspecialchars($row['nom']) . "</strong>.";
        } else {
            // Préparer JSON des types (si aucun type coché, on met tableau vide)
            if (!is_array($types)) {
                $types = [];
            }
            $typesJson = json_encode(array_values($types));

            // Insert avec les types
            $stmt = $pdo->prepare("INSERT INTO caracteristiques (nom, types) VALUES (?, ?)");
            $stmt->execute([$nom, $typesJson]);

            $message = "Caractéristique ajoutée avec succès ✔️";

            // Reset champ après ajout
            $nom   = "";
            $types = [];
        }
    }
...
<head>
    <meta charset="UTF-8">
    <title>Ajouter Caractéristique - DWIRA</title>
...
<div class="main">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap">
        <h2 class="fw-bold mb-2 mb-md-0">➕ Ajouter une caractéristique</h2>
        ...
        <form method="POST">
            ...
            <div class="mb-3">
                ...
                <input 
                    ...
                    name="nom" 
                    class="form-control"
                    placeholder="Ex : Vue mer, Proche plage, Gaz de ville..."
                    value="<?= htmlspecialchars($nom) ?>"
                    required
                >
                <small class="hint">
                    Ce nom apparaîtra comme <strong>badge</strong> dans les fiches Biens & Demandes.
                </small>
            </div>
            ...
            <div class="mb-3">
                ...
                <small class="hint">
                    Si tu coches par exemple <strong>Appartement</strong> et <strong>Villa</strong> pour « Vue mer »,  
                    ça sert à filtrer automatiquement les caractéristiques par type dans les formulaires Biens & Demandes.
                </small>
            </div>
