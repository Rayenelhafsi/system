...
// Récupérer toutes les caractéristiques avec la colonne `types` (JSON)
...
// Liste des villes (même que pour les biens)
$villes = [
    "Kelibia",
    "Manzel Tmim",
    "Hammem Ghzez",
...
// Liste type de rue (même que pour biens)
$typeRueOptions = [
    "Piste",
    "Route goudronnée",
    "Rue résidentielle"
...
// Liste type de papier (même que pour biens)
$typePapierOptions = [
...
// Décoder les caractéristiques déjà sauvées
$demandeCaracteristiques = json_decode($demande['caracteristiques'] ?? "[]", true);
if (!is_array($demandeCaracteristiques)) {
    $demandeCaracteristiques = [];
}
...
// Type de demande courant
$typeDemande = $demande['type_bien'] ?? 'Appartement';
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($typeDemande === 'Appartement') {
        ...
        $details['config_souhaitee']     = $_POST['config_appart']           ?? null;
    ...
    // Rafraîchir les données pour réafficher le formulaire à jour
    $stmt = $pdo->prepare("SELECT * FROM clients_demandes WHERE id = ?");
    $stmt->execute([$id]);
    $demande = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    // Relancer le matching car budget / ville / type etc. ont pu changer
    MatchService::matchWithBiens($pdo, $id);

    $message = "Demande mise à jour avec succès ✔️";
}
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <title>Éditer une Demande - DWIRA</title>
...
<div class="main">
    <h2>✏️ Éditer la Demande #<?= $demande['id'] ?></h2>
        ...
        <form method="POST">
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="nom" class="form-control"
                           required value="<?= htmlspecialchars($demande['nom']) ?>">
                </div>
                <div class="col-md-6">
                    <label>Téléphone</label>
                    <input type="text" name="telephone" class="form-control"
                           required value="<?= htmlspecialchars($demande['telephone']) ?>">
                </div>
            </div>
            ...
            <div class="row mb-3">
                <div class="col-md-4">
                    ...
                    <input type="number" name="budget" class="form-control"
                           required value="<?= htmlspecialchars($demande['budget_max']) ?>">
                </div>
                <div class="col-md-4">
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" name="chambres" class="form-control"
                           required value="<?= htmlspecialchars($demande['chambres_min']) ?>">
                </div>
            </div>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <select name="type" class="form-select" id="typeDemande">
                        <option <?= $demande['type_bien'] === 'Appartement' ? 'selected' : '' ?>>Appartement</option>
                        <option <?= $demande['type_bien'] === 'Villa' ? 'selected' : '' ?>>Villa</option>
                        <option <?= $demande['type_bien'] === 'Terrain' ? 'selected' : '' ?>>Terrain</option>
                        <option <?= $demande['type_bien'] === 'Local commercial' ? 'selected' : '' ?>>Local commercial</option>
                        <option <?= $demande['type_bien'] === 'Immeuble' ? 'selected' : '' ?>>Immeuble</option>
                    </select>
                </div>
                <div class="col-md-6">
                    <label>Statut</label>
                    <select name="statut" class="form-select">
                        <option <?= $demande['statut'] === 'Vente' ? 'selected' : '' ?>>Vente</option>
                        <option <?= $demande['statut'] === 'Location' ? 'selected' : '' ?>>Location</option>
                    </select>
                </div>
            </div>
                ...
                <div class="col-md-3 d-flex flex-column justify-content-end mb-3">
                    <div class="form-check mb-1">
                        <input class="form-check-input" type="checkbox" name="constructible_terrain" id="constructible_terrain"
                               <?= !empty($details['constructible_souhaite']) ? 'checked' : '' ?>>
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="coin_angle_terrain" id="coin_angle_terrain"
                               <?= !empty($details['coin_angle_souhaite']) ? 'checked' : '' ?>>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="number" name="etage_min_appart" class="form-control"
                               value="<?= htmlspecialchars($details['etage_min'] ?? '') ?>">
                    </div>
                    <div class="col-md-3">
                        <label>Étage max</label>
                        <input type="number" name="etage_max_appart" class="form-control"
                               value="<?= htmlspecialchars($details['etage_max'] ?? '') ?>">
                    </div>
                </div>
                ...
                <div class="row mb-3">
                    <div class="col-md-3">
                        ...
                        <input type="number" name="nb_chambres_min_villa" class="form-control"
                               value="<?= htmlspecialchars($details['nb_chambres_min'] ?? '') ?>">
                    </div>
                    <div class="col-md-3">
                        <label>SDB min</label>
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="number" name="nb_etages_min_villa" class="form-control"
                               value="<?= htmlspecialchars($details['nb_etages_min'] ?? '') ?>">
                    </div>
                    <div class="col-md-3">
                        <label>Étages max</label>
                        <input type="number" name="nb_etages_max_villa" class="form-control"
                               value="<?= htmlspecialchars($details['nb_etages_max'] ?? '') ?>">
                    </div>
                </div>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="number" step="0.01" name="hauteur_min_local" class="form-control"
                               value="<?= htmlspecialchars($details['hauteur_min_m'] ?? '') ?>">
                    </div>
                </div>
            ...
            <div class="mb-3">
                ...
                <div class="d-flex flex-wrap" id="caracs-container-demande">
                    <?php foreach($caracteristiquesOptions as $option): ?>
                        <?php
                            ...
                            $nomCarac = $option['nom'];
                            $checked  = in_array($nomCarac, $demandeCaracteristiques, true);
                        ?>
                ...
                <small class="text-muted">
                    Affiche les caractéristiques dont le type est compatible
                    avec le type choisi, plus celles vraiment <strong>Global</strong>.
                </small>
            </div>

            <button type="submit" class="btn btn-warning">
                <i class="bi bi-pencil-square"></i> Mettre à jour la demande
            </button>
        </form>
    </div>
</div>
